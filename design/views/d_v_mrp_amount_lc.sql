...
WITH
    ...
    (
        ...
        FROM
            D_T2_STORAGE st
        RIGHT JOIN
            D_T2_BRANCH_STORAGE bs
        ON
            bs.STORAGE_ID = st.STORAGE_ID
        INNER JOIN
            lc
        ON
            lc.BRANCH_ID = bs.BRANCH_ID
        GROUP BY
            bs.BRANCH_ID,
            st.ITEM_ID
    )
...
LEFT JOIN
    real_count r
ON
    r.BRANCH_ID = lc.BRANCH_ID
AND i.ITEM_ID = r.ITEM_ID
LEFT JOIN
    D_T2_SUPPLY_CYCLE s
ON
    s.ITEM_ID = i.ITEM_ID
AND s.BRANCH_ID = lc.BRANCH_ID
LEFT JOIN
    D_T2_SUPPLIER_BRANCH_ITEM sbi
ON
    lc.BRANCH_ID = sbi.BRANCH_ID
AND sbi.ITEM_ID = i.ITEM_ID
AND sbi.PRIORITY = 0
LEFT JOIN
    D_T2_PRODUCTION_CYCLE pc
ON
