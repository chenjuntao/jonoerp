...
public class AuditCross {
	...
	public void testTt() throws Exception {
		...
		// 审核采购单
		tester.clickLeftMenu("库存基础管理", "越库配送单审核");
		tester.switchRightTab("越库配送单审核");
		...
		tester.switchRightTabLike("审核越库配送单-");
	...
	public void testAuditCross(Tester tester,WebDriver driver,Logger logger) throws Exception {
		tester.clickLeftMenu("库存基础管理", "越库配送单审核");
		tester.switchRightTab("越库配送单审核");
		driver.findElement(By.id("startDate")).clear();
		driver.findElement(By.id("startDate")).sendKeys("2016-09-18");
		driver.findElement(By.id("endDate")).clear();
		driver.findElement(By.id("endDate")).sendKeys("2016-09-25");
		Thread.sleep(1000);
		driver.findElement(By.xpath("//input[@value='查    询']")).click();
		Thread.sleep(1000);
		if (!doesWebElementExist(driver, By.id("operate1"))){
			System.out.println("生成采购单之后未生成越库配送单");
			logger.debug("生成采购单之后未生成越库配送单");
		}
		driver.findElement(By.id("operate1")).click();
		if (tester.isAlertPresent()) {
			tester.closeAlertAndGetItsText();
		...
		if (tester.isAlertPresent()) {
			if (tester.closeAlertAndGetItsText().indexOf("失败")!=-1)
				logger.debug("越库配送单审核失败");
		}
		logger.debug("越库配送单审核完成");
	}
