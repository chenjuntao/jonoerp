 ...
 * 说明： 
 */
public class AcceptInputCollect {
	@Before
	...
	public void testRequire() throws Exception {
		tester.clickLeftMenu("物流中心报表管理", "物流中心验收入库汇总报表");
		Thread.sleep(1000);
		tester.switchRightTab("物流中心验收入库汇总报表");
		if (driver instanceof JavascriptExecutor) {
			...
			if (!condition) logger.debug("物流中心验收入库汇总报表grid初始化失败");
			Assert.assertTrue(condition);
		}
		
		Thread.sleep(1000);
		tester.closeRightTab("物流中心验收入库汇总报表");
		Thread.sleep(1000);
	}
}
