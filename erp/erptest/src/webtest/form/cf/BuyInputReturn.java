...
public class BuyInputReturn {
	...
	public void testRequire() throws Exception {
		tester.clickLeftMenu("报表管理", "中央工厂采购入库退货报表");
		Thread.sleep(1000);
		tester.switchRightTab("中央工厂采购入库退货报表");
		if (driver instanceof JavascriptExecutor) {
			...
			if (!condition) logger.debug("中央工厂采购入库退货报表grid初始化出错");
			Assert.assertTrue(condition);
		}
		
		Thread.sleep(1000);
		tester.closeRightTab("中央工厂采购入库退货报表");
		Thread.sleep(1000);
	}
}
