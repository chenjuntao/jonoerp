 ...
 * 说明： 
 */
public class NoworkAchieveDetail {
	@Before
	...
	public void testRequire() throws Exception {
		tester.clickLeftMenu("报表管理", "中央工厂非工单领料明细报表");
		Thread.sleep(1000);
		tester.switchRightTab("中央工厂非工单领料明细报表");
		if (driver instanceof JavascriptExecutor) {
			...
			if (!condition) logger.debug("中央工厂非工单领料明细报表grid初始化出错");
			Assert.assertTrue(condition);
		}
		
		Thread.sleep(1000);
		tester.closeRightTab("中央工厂非工单领料明细报表");
		Thread.sleep(1000);
	}
}
