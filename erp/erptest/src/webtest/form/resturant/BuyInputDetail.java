...
//	@Before
	...
	public void testRequire() throws Exception {
		tester.clickLeftMenu("餐厅报表管理", "餐厅采购入库明细报表");
		Thread.sleep(1000);
		tester.switchRightTab("餐厅采购入库明细报表");
		if (driver instanceof JavascriptExecutor) {
			...
			if (!condition) logger.debug("餐厅采购入库明细报表grid初始化失败");
			Assert.assertTrue(condition);
		}
		
		Thread.sleep(1000);
		tester.closeRightTab("餐厅采购入库明细报表");
		Thread.sleep(1000);
	}
}
