...
<div style="width:100%;overflow:auto;">
	...
	<table id="shopTable" class="hovertable table-striped" width="1800" border="1" align="center"
		...
		<c:set var="payAmtTal" value="${0}"/>  
			...
			<tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';" align="center">
				...
				<td><a
					href='javascript: gotoURL("<%=appRoot %>/businessquery/tableListQuery/listTable.action?shopC=${item.shopC}&startDate=${entry.key}&endDate=${entry.key}");'
					>${item.shopC}</a>
				</td>
				<td >${item.shopName}</td>
				<td><a
					href='javascript: gotoURL("<%=appRoot %>/businessquery/paymentquery/doShopPayQuery.action?shopC=${item.shopC}&startDate=${entry.key}&endDate=${entry.key}");'>付款方式
				</a></td>
