...
<script type="text/javascript"
	src="<%=appRoot%>/jsp/restaurant/prototype/allocateitem/processallocateitembill/processAllocateItemBill.js?Version=<%=currenttime %>"></script>
...
<input name="agreeDate" type="text" id="agreeDate" />
	...
	<div id="dataGrid">输入实际调出数量，会自动计算出调拨差异（申请调拨数量-调出数量）</div>
	
	<label>
	<input name="processAllocateItemBill" type="submit" id="processAllocateItemBill" value="餐厅进行调拨" />
	</label>
  （注：餐厅调拨单状态从未拨付变为已拨付；调出餐厅库存减去调出数量；调入餐厅增加调出数量）
	</form>
</body>
