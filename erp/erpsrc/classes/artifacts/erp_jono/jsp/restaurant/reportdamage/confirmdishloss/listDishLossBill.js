...
var grid = null;
function initGrid() {
	var _url = appRoot + "/restaurant/reportdamage/confirmdishloss/doQuery.action?branchType="
			+ dojo.byId('branchType').value;
	_url = getUrl(_url);
...
function doAudit(row) {
	...
	var _url = appRoot + "/restaurant/reportdamage/confirmdishloss/auditView.action?formId=" + row.formId
			+ "&parentTabId=" + tabId + "&branchType=" + dojo.byId('branchType').value;
	_url = getUrl(_url);

	addTab("审核出品报损单-" + row.formId, _url);
}