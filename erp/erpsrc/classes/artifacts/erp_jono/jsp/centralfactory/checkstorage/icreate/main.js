...
function addList() {
	...
	var _url = appRoot + '/restaurant/checkstorage/icreate/checkView.action?parentTabId=' + tabId;
	_url = getUrl(_url);

	addPostTab('billForm', '盘点清单生成确认', _url);
}
...
function initGrid() {
	require([ "dgrid/OnDemandGrid", "dojo/store/Memory", "dojo/_base/declare", "dojo/query", "dgrid/Keyboard",
			"dgrid/editor", "dojo/domReady!" ], function(OnDemandGrid, Memory, declare, query, Keyboard, editor) {
...
function pickModel() {
	if (pickModelDlg == null) {
		var _url = appRoot
				+ "/lc/checkstorage/icreate/pickModelView.action?templateType=checkstorage&branchType=CENTRALFACTORY";
		_url = getUrl(_url);
