...
function doSubmit() {
	...
	var _url = appRoot + '/centralfactory/productionDemand/purchaseBill/treeCreate/checkView.action?parentTabId='
			+ tabId;
...
function initGrid() {
	...
	var _url = appRoot + "/centralfactory/productionDemand/purchaseBill/treeCreate/queryMRP.action";
	$.ajax({
		type : "POST",
		url : _url,
		data : {
...
var colHeaders = [ '原料编码', '原料名称', '主库位', '类别', '单位', '规格', '最小订货倍量', '采购量', '日均订货量1', '日均订货量2', '日均订货量3', '当前要货量',
		'供货周期', '库存周期', '安全库存1', '安全库存2', '安全库存3', '默认安全库存', '已分配量', '在途量', '系统库存', '建议1', '建议2', '建议3', '默认建议值',
		'标准价', '进货价' ];
