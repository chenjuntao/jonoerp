...
var grid = null;
function initGrid() {
	var _url = appRoot + "/cf/requisition/manage/doQuery.action?queryType=unreceive&formType=manual";
	_url = getUrl(_url);
...
function getColumn() {
	return [ [ [ {
		...
		renderCell : function(object, data) {
			return hrefFmt("领料", doConfirm, object);
		}
	}, {
		label : "制单人员",
		field : "formMaker"
...
function doConfirm(row) {
	...
	// return;
	// }
	var _url = appRoot + "/cf/requisition/manual/confirm/confirmView.action?formId=" + row.formId + "&parentTabId="
			+ tabId;
	_url = getUrl(_url);
