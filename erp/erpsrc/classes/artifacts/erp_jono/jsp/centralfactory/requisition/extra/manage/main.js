...
var grid = null;
function initGrid() {
	var _url = appRoot + "/cf/requisition/manage/doQuery.action?formType=extra";
	_url = getUrl(_url);
...
function doEdit(row) {
	...
	var _url = appRoot
			+ "/cf/requisition/manage/editView.action?formId="
			+ row.formId + "&parentTabId=" + tabId;
	_url = getUrl(_url);
	
	var _title = '管理超领单-' + row.formId;
	addTab(_title, _url);
}
