...
/*var storageId = dojo.byId('storageId').value;
	...
	if (storageId.trim() == '' && g_branchType != "OUTERORDER") {
		alert('仓库编码不能为空！');
	...
	if (storageName.trim() == '' && g_branchType != "OUTERORDER") {
		alert('仓库名称不能为空！');
		dojo.byId('storageName').focus();
		return false;
	}*/
	return true;
}
