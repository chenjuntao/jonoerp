...
<script type="text/javascript" src="<%=appRoot %>/jsp/businessquery/paymentquery/export_bill.js"></script>
</head>
...
<div id="content">
		<table class="hovertable" width="100%" border="1" cellpadding="6" cellspacing="2"
			...
			<tr valign="middle">
				...
				<td  width="200">&nbsp;付款方式：${payName}</td>
			</tr>
			
		<input type="hidden"  id="startDate" name="startDate" value="${startDate}"/>
		<input type="hidden"  id="endDate"	name="endDate" value="${endDate}"/>
		<input type="hidden"  id="shopC"	name="shopC" value="${shopC}"/>
		<input type="hidden"  id="payType"	name="payType" value="${type}"/>
		</table>
		
<div id="pagelist">
		<button onclick='require([ "dojo/_base/xhr" ], function(xhr) {  
	        xhr.post({
		    	url : "pagePaymentBillList.action?shopC=${shopC}&shopName=${shopName}&startDate=${startDate}&endDate=${endDate}&type=${type}&currentPage=1&pageCount=${pageCount}",
	            handleAs : "text",  
	            load : function(data) {
					dojo.byId("paymentBillListResult").innerHTML = data;
		...
		<button onclick='require([ "dojo/_base/xhr" ], function(xhr) {
	        ...
	        xhr.post({
		    	url : "pagePaymentBillList.action?shopC=${shopC}&shopName=${shopName}&startDate=${startDate}&endDate=${endDate}&type=${type}&currentPage="+newPage+"&pageCount=${pageCount}",
	            handleAs : "text",  
	            load : function(data) {
					dojo.byId("paymentBillListResult").innerHTML = data;
    	...
    	<button onclick='require([ "dojo/_base/xhr" ], function(xhr) {  
	        xhr.post({
		    	url : "pagePaymentBillList.action?shopC=${shopC}&shopName=${shopName}&startDate=${startDate}&endDate=${endDate}&type=${type}&currentPage=${pageCount}&pageCount=${pageCount}",
	            handleAs : "text",  
	            load : function(data) {
					dojo.byId("paymentBillListResult").innerHTML = data;
			...
			<table width="1200" class="hovertable" border="1" align="left"
				...
				<c:forEach var="bill" items="${bills}">
					...
					<tr onmouseover="this.style.backgroundColor='#ffff66';"
						...
						<td><fmt:formatNumber type="number"
								value="${bill.payAmt}"  maxFractionDigits="2"/></td>
				</c:forEach>
				<tr onmouseover="this.style.backgroundColor='#ffff66';"
					onmouseout="this.style.backgroundColor='#d4e3e5';" align="center">
					<td><b>合计：</b></td>
