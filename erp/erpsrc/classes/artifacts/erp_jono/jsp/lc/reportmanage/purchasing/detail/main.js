...
function getAppendValue(){
	...
	var supplierCondition = dijit.byId('supplierCondition').get('value');
	...
	if(!isEmpty(branchCondition)){
		appendValue += " AND h.REQUESTER_ID = '" + branchCondition+ "' "
	}
	
	if(!isEmpty(supplierCondition)){
		appendValue += " AND h.PROVIDER_ID = '" + supplierCondition+ "' "
	}
...
function initCondition(){
	...
	dijit.byId('supplierCondition').set('value',"");
		...
		xhr.post(appRoot + "/common/listBs.action", {
			...
			data : {
			 ...
			 array.forEach(data.branch, function(item){
				 ...
				 if(item.branchType == "SUPPLIER" || item.branchType == "CENTRALFACTORY"){
					 supplierArr.push(item);
				 }
				 
				 if(item.branchType == "RESTAURANT" || item.branchType == "OUTERORDER"){
					 receiverArr.push({label:item.name,value:item.code});
				 }
			 });
			 
			 new FilteringSelect({
			        id: "supplierCondition",
			        store: new Memory({idProperty: "code", data: supplierArr }),
			        value:"code",
			        required : false,
					autoComplete : false,
			        query: {branchId: /.*/},
			    ...
			    new FilteringSelect({
					...
					autoComplete : false,
					queryExpr: "*${0}*",
			        store: new Memory({ idProperty: "code", data: requesterArr }),
			        style: "width: 150px;"
...
var getExportArgs = function(){
	...
	for(var i = 0; i < c.length; i++){
		if(c[i].checked){
			if(c[i].id == 'delivery_type'){
				columns.push('delivery_text');
			}else{
				columns.push(c[i].id);
			}
			names.push(c[i].name);
		...
		grid = new Grid({
			...
			structure : [
							...
							},
							{
								...
								autoComplete : false,
								dataType : 'enum',
								enumOptions : [{label:'直配',value:'DIRECT'},{label:'统配',value:'UNIFIED'},{label:'越库',value:'CROSS'} ],
