...
function getAppendValue() {
	...
	appendValue += " AND to_char(input_time, 'yyyy-mm') != to_char(audit_time, 'yyyy-mm') "
...
require([ "dojo/dom", "gridx/Grid", "gridx/core/model/cache/Async", "gridx/modules/Pagination",
		...
		Pagination, PaginationBar, Filter, FilterBar, Server, NestedSort, VirtualVScroller, ColumnResizer, ColumnLock) {
	initGrid = function() {
...
function getStructure() {
	return [
			...
			},
			{
				...
				name : "配送单号",
				autoComplete : false,
				disabledConditions : [ 'equal', 'startWith', 'endWith', 'notEqual', 'notContain', 'notStartWith',
						'notEndWith', 'isEmpty' ],
				decorator : function(cellData, rowId, rowIndex) {
					if (!cellData) {
						return '';
