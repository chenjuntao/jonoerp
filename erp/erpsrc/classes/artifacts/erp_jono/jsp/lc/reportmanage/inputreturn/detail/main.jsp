...
<head>
	...
	<script type="text/javascript" src="<%=appRoot %>/jsp/lc/reportmanage/inputreturn/detail/main.js?Version=<%=currenttime %>"></script>
	<script type="text/javascript" src="<%=appRoot %>/jsp/lc/reportmanage/inputreturn/detail/export.js?Version=<%=currenttime %>"></script>
	<script type="text/javascript" src="<%=appRoot %>/jsp/lc/reportmanage/inputreturn/detail/print.js?Version=<%=currenttime %>"></script>
	...
	<link rel="stylesheet" href="<%=appRoot %>/jsp/lc/reportmanage/inputreturn/detail/main.css">
	<link rel="stylesheet" href="<%=appRoot %>/jsp/lc/reportmanage/inputreturn/detail/print.css">
...
<form id="dataForm" method="post" >
	   <table class="hovertable" width="100%" border="1" style="height: 35px;">
	   			<td class="label_right" style="width: 90px;">入库\退货日期：</td>
				<td class="text_left"  style="width: 270px;">
					<input type="text" class="Wdate" id="startDate" value="${startDate}" style="width: 100px;"	onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'endDate\',{d:0});}'})" /> 
					<span style="padding: 0 5px;">至</span> 
					<input	type="text" class="Wdate" id="endDate" value="${endDate}"	style="width: 100px;" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'startDate\',{d:0});}'})" />
				</td>
				
				<td  style="width: 180px;">
					<label for="branchCondition">入库门店：</label>
					<input id="branchCondition" />
				</td>
				
				<td style="width: 200px;">
					 <label for="storageCondition">入库仓库：</label>
		...
		<input id="branchType"   name="branchType" type="hidden" value="LOGISTICSCENTER"/>
		<input name="reportFlag" type="hidden" value="D_T1_INPUT_DETAILS"/>
