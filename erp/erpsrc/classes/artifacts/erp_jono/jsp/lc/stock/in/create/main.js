...
function doQuery(_param) {
	...
	var _url = appRoot + "/lc/stock/in/create/loadTree.action";
	_url = getUrl(_url);
...
function loadData(data) {
	...
	if (deliveryType == 'CROSS') {
		dType = '越库';
		grid.styleColumn('receiver', "display: table-cell;");
	} else {
		grid.styleColumn('receiver', "display: none;");
	}
...
function putinStorage() {
	...
	var _url = appRoot + '/lc/stock/in/create/createView.action?formId=' + formId + "&supplierId=" + supplierId
			+ "&deliveryType=" + deliveryType + "&branchId=100";
	_url = getUrl(_url);
