...
<head>
	...
	<script type="text/javascript" src="<%=appRoot %>/jsp/hq/stationobservation/export.js?Version=<%=currenttime %>"></script>
...
<body class="claro" style="overflow-y: auto;overflow-x: hidden;">
	...
	<form id="billForm" method="post" >
		<table class="hovertable" width="100%" border="1">
			<tr>
				<td>
					 <div id="leftTitle"  data-dojo-type="dijit/InlineEditBox" data-dojo-props="editor:'dijit/Editor',editing:true, renderAsHtml:true, autoSave:true,
...
<!-- 			第一部分：准备工作 -->
			...
			<tr>
				<td rowspan="2" colspan="1">
					 <div style="height: 300px" id="aPartContent" name="aPartContent" data-dojo-type="dijit/InlineEditBox" data-dojo-props="editor:'dijit/Editor', editing:true,renderAsHtml:true, autoSave:true,
   							 editorParams:{ extraPlugins: []}">${aPartContent }</div>
				</td>
				<td colspan="1">
				<table >
					<div id="dataGrid" style="height: 0px;width:250px">
					<tr>
					<div style="text-align: center;" >
							<span style="font-size: 2em;">
								成本卡
					...
					<c:forEach items="${itemLst }" var="item">
					<tr>
						<td >${item.itemId }</td>
						<td >${item.itemName }</td>
...
<!-- 			第二部分：示范说明           （按照1份的量配备制作） -->
			...
			<tr>
				...
				<td colspan="2">
					<div style="height: 350px;width:800px ;overflow-x:scroll;">
						<table id="aTable" >
							<c:forEach items="${aTable }" var="item" varStatus="i">
							<td>
						第${i.index+1 }步：${item.stepRemark }
						</td>
					</c:forEach>
						</table>
					</div> 
			...
			<tr>
				...
				<td colspan="2">
					<div style="height: 350px;width:800px ;overflow-x:scroll;">
						<table id="bTable" >
								<c:forEach items="${bTable }" var="item" varStatus="i">
							<td>
						第${i.index+1 }步：${item.stepRemark }
						</td>
					</c:forEach>
			...
			<tr>
				...
				<td colspan="2">
					<div style="text-align: center;" >
						<div style="height: 350px;width:800px ;overflow-x:scroll;">
							<table id="cTable" >
								<c:forEach items="${cTable }" var="item" varStatus="i">
							<td>
						第${i.index+1 }步：${item.stepRemark }
						</td>
					</c:forEach>
