...
<head>
	...
	<script type="text/javascript" src="<%=appRoot %>/jsp/hq/config/pgroup/lc/edit.js?Version=<%=currenttime %>"></script>
...
<body class="claro" style="overflow-y: auto;overflow-x: hidden;">
	<form id="dataForm" method="post" >
		<input type='hidden' name='priceGroup.priceGroupType' value='BRAND' />
		<table class="hovertable" width="100%" border="1">
			...
			<tr>
				<td class="label_right" style="width: 100px;">所属门店品牌：</td>
				<s:if test="%{priceGroupId == null}">
					<td class="text_left">
						<s:select list="brandLst" listKey="brandId" listValue="brandName" theme="simple"
							 style="width: 150px;" id="owner" name="priceGroup.owner"
			 				 value="%{priceGroup.owner}"></s:select> 
			 		</td>
					<td class="label_right">参考价格组：</td>
			 		<td>
						<s:select list="refGroupLst" listKey="priceGroupId" listValue="priceGroupName" theme="simple"
							 style="width: 150px;" id="owner" name="refGroupId"
			 				 value=""></s:select> 
			 		</td>
				</s:if>
     			<s:else>
					<td class="text_left" colspan="3">
						<input type='hidden' name='priceGroup.owner' value='${priceGroup.owner}' />
						<span>${priceGroup.ownerName }</span>
     			</s:else>
				</td>
			</tr>
		</table>
