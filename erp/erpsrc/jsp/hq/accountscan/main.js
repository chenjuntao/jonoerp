...
function getColumn() {
	return [ {
		...
		renderCell : function(object, data) {
			...
			if (initStatus == 'supplier_confirmed') {
				return hrefFmtId("付款", doOperate, object, "operate" + object.rownumber);
			}

			if (initStatus == 'payed') {
				return hrefFmtId("收款确认", doOperate, object, "operate" + object.rownumber);
			}
...
function doOperate(row) {
	...
	var _url = appRoot + "/hq/acscan/operate.action?formId=" + row.formId + "&operate=" + operate + "&status="
			+ initStatus + "&parentTabId=" + tabId;
