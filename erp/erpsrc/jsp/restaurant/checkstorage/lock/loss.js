...
function LossGrid(gridId) {
	...
	this.init = function(_query) {
		var _url = appRoot
				+ "/restaurant/reportdamage/queryloss/queryByStatus.action?queryType=RAWLOSS&statusType=ap&branchType="
				+ branchType;
		_url = getUrl(_url);
	...
	this.doAudit = function(row) {
		...
		if (result.hasLock) {
			...
			var _url = appRoot + "/restaurant/reportdamage/confirmloss/auditView.action?formId=" + row.formId
					+ "&parentTabId=" + tabId;
			_url = getUrl(_url);
	...
	this.doProcess = function(row) {
		...
		if (result.hasLock) {
			...
			var _url = appRoot + "/restaurant/reportdamage/processloss/processView.action?formId=" + row.formId
					+ "&parentTabId=" + tabId;
			_url = getUrl(_url);
