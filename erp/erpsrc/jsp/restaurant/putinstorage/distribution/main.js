...
var grid = null;
function initGrid() {
	var _url = appRoot + "/restaurant/putinstorage/distribution/doQuery.action?branchType=RESTAURANT";
	_url = getUrl(_url);
	...
	require([ "dgrid/Grid", "custom/store/Server","dojo/store/Observable", "dojo/store/Cache", "dojo/store/Memory",
			...
			Cache, Memory,ColumnResizer,declare,ColumnSet,Pagination) {
...
function getColumn() {
	return [
		...
		field : 'operate',
		renderCell : function(object, data) {
			return hrefFmtId("配送单入库", doEdit, object, "operate"+object.rownumber);
		},
		sortable:false
	}] ], [ [ {
		label : '订货部门',
...
function doEdit(row) {
	...
	var _url = appRoot + "/restaurant/putinstorage/distribution/editView.action?formId="
			+ row.formId + "&parentTabId=" + tabId;
	_url = getUrl(_url);
	
	var _title = '配送单入库-' + row.formId;
	addTab(_title, _url);
}