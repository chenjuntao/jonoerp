...
<head>
	...
	<script type="text/javascript" src="<%=appRoot%>/jsp/restaurant/production/creategoods/createSelf.js?Version=<%=currenttime %>"></script>
...
<body class="claro" style="overflow: hidden;">
	...
	<form id="createLossForm" method="post">
		...
		<table class="hovertable" width="100%" border="0">
			<tr>
				<th colspan="5"><b>选择半成品生产入库</b></th>
			</tr>
			
			<tr>
				<td>
					<input name="DIYchoose" type="button" id="DIYchoose" value="选择半成品" onclick="selMaterial()"/>
				</td>
				
				<td>
					生产部门：
					<s:select list="shopList" listKey="code" listValue="name" theme="simple" style="width: 180px;" id="branchId" name="selfSemisHeader.branchId" onchange="refreshStorage()"></s:select>
					<input type="hidden" id="branch" name="selfSemisHeader.branch"  />
				</td>
...
<!-- 					<input type="hidden" id="storage" name="selfSemisHeader.storage" /> -->
<!-- 				</td> -->
		...
		<table class="hovertable" border="0" id="commandTable">
			<tr>
				<td class="text_left" colspan="4" style="text-align: center;">
					<input type="button" value="生成半成品生产单" onclick="createLoss()" />
				</td>
			</tr>
		</table>
	</form>
