...
function getAppendValue() {
	...
	// -----------------------------------------------------------------------------
	var appendValue0 = " AND b.BRANCH_TYPE = 'RESTAURANT'  AND io.BRANCH_ID = '" + loginBranchId + "' ";
	if (!isEmpty(branchFlag)) {
		appendValue0 = " AND b.BRANCH_TYPE = 'RESTAURANT' ";
	}
	...
	if (!isEmpty(branchCondition)) {
		appendValue0 += " AND io.BRANCH_ID = '" + branchCondition + "' "
	}

	// ---------------------------------------------------------------------

	var appendValue1 = " AND b.BRANCH_TYPE = 'RESTAURANT'  AND io.BRANCH_ID = '" + loginBranchId + "' ";
	if (!isEmpty(branchFlag)) {
		appendValue1 = " AND b.BRANCH_TYPE = 'RESTAURANT' ";
	}
	...
	if (!isEmpty(branchCondition)) {
		appendValue1 += " AND io.BRANCH_ID = '" + branchCondition + "' "
	}

	// --------------------------------------------------------------------
	var appendValue2 = " AND b.BRANCH_TYPE = 'RESTAURANT' AND io.BRANCH_ID = '" + loginBranchId + "' ";
	if (!isEmpty(branchFlag)) {
		appendValue2 = " AND b.BRANCH_TYPE = 'RESTAURANT' ";
	}
	...
	if (!isEmpty(branchCondition)) {
		appendValue2 += " AND io.BRANCH_ID = '" + branchCondition + "' "
	}
...
function init() {
	require([ "dojo/dom", "dojo/request/xhr", "dojo/_base/array", "dojo/dom-form", "dojo/parser", "dijit/form/Select",
			"dojo/data/ObjectStore", "dojo/store/Memory", "dijit/form/FilteringSelect", "dojo/date",
			"dojo/date/locale", "dijit/form/Button", "dijit/Dialog", "dojo/domReady!" ], function(dom, xhr, array,
			domForm, parser, Select, ObjectStore, Memory, FilteringSelect, date, locale) {
		parser.parse();
