...
function init() {
	require([ 
	          ...
	          "dijit/Dialog",
	          "dojo/domReady!" 
	  ], function(dom, xhr, array,domForm,parser,Memory,FilteringSelect) {
		parser.parse();
		...
		window.getQuery = function() {
			return {
				...
				tagId:dijit.byId('tagCondition').get('value')
			};
		}
...
function doScan(item) {
	...
	+ "&endDate="+dojo.byId('endDate').value+"&tagId="+dijit.byId('tagCondition').get('value');
...
function initGrid() {
	var _url = appRoot + "/reportmanage/restaurant/cost/jono/doQuery.action";
	_url = getUrl(_url);
