...
function getAppendValue() {
	...
	var appendValue = " AND h.CHECK_BRANCH_ID = '" + loginBranchId + "' "
	...
	if (!isEmpty(branchCondition)) {
		appendValue += " AND h.CHECK_BRANCH_ID = '" + branchCondition + "' "
	}

	if (!isEmpty(storageCondition)) {
		appendValue += " AND h.CHECK_STORAGE_ID = '" + storageCondition + "' "
	}
	return appendValue;
}
