...
function createPrintTable(id) {
	...
	require(
			...
			function(xhr) {
				xhr
						.post(appRoot + "/reportmanage/common/print/print.action", {
							handleAs : "json",
							data : getQuery()
						})
						.then(
								function(data) {
									...
									for (j = 0, l = data.length; j < l; j++) {
										...
										for (i = 0; i < printObject.columns.length; i++) {
											if (data[j].hasOwnProperty(printObject.columns[i])) {
												if (data[j][printObject.columns[i]] === undefined
														|| data[j][printObject.columns[i]] === null) {
									...
									LODOP.ADD_PRINT_TEXT(20, 300, 134, 20, "餐厅盘点明细报表");
									...
									var strFormHtml = strBodyStyle + "<body>"
											+ document.getElementById("grid_print").innerHTML + "</body>";
