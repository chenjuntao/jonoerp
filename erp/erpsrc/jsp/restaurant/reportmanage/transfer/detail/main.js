...
function getAppendValue() {
	...
	if (!isEmpty(branchCondition) || !isEmpty(storageCondition) || !isEmpty(outBranchCondition) || !isEmpty(outStorageCondition)){
		appendValue += " AND ( 1 > 1 "
	}

	var tag = 0;
	if (!isEmpty(branchCondition)) {
		appendValue += " OR ( h.IN_BRANCH_ID = '" + branchCondition + "' "
		tag = 1;
	}
	...
	if (!isEmpty(storageCondition)) {
		if (tag == 1)
			...
			appendValue += "OR h.IN_STORAGE_ID = '" + storageCondition + "' "
	}else{
		if (tag == 1) appendValue += ")"
	}

	tag = 0;
	if (!isEmpty(outBranchCondition)) {
		appendValue += " OR ( h.OUT_BRANCH_ID = '" + outBranchCondition + "' "
		tag = 1;
	}
	...
	if (!isEmpty(outStorageCondition)) {
		if (tag == 1)
			...
			appendValue += " OR h.OUT_STORAGE_ID = '" + outStorageCondition + "' "
	}else{
		if (tag == 1) appendValue += ")"
	}
