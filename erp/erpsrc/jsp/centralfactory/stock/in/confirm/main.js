...
function getAppendValue() {
	...
	var appendValue = "  AND s.STATUS = '未审核' AND b.BRANCH_TYPE = 'CENTRALFACTORY'  AND (h.FORM_TYPE = 'PRODUCE') ";
...
function doConfirm(row) {
	...
	var _url = appRoot + "/cf/stock/in/confirm/confirmView.action?formId=" + row.formId + "&parentTabId=" + tabId;
...
function checkFinish(row) {
	...
	require([ "dojo/request/xhr" ], function(xhr) {
		xhr.post(_url, {
			...
			if (!data.finished) {
				alert('还存在未审核或未处理的退料单或超领单，不能入库审核!');
			} else {
				doConfirm(row);
			}
		}, function(err) {
