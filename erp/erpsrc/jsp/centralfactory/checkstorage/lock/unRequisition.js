var unRequisitionGrid = new UnRequisitionGrid('unRequisitionGrid');

function UnRequisitionGrid(gridId) {
	var instance = this;
	...
	this.init = function(_query) {
		var _url = appRoot + "/cf/requisition/manage/doQuery.action?queryType=unaudit&formType=manual&branchType="+branchType;
		_url = getUrl(_url);
	...
	this.doAudit = function(row) {
		...
		if (result.hasLock) {
			...
			addTab("审核非工单领料单-" + row.formId, _url);
		} else {
