...
function getTempHref(templateId) {
	return "<a href='#' onclick='doScanTemp(\"" + templateId + "\")'><b>"
			+ templateId + "</b></a>";
}
...
function initGrid() {
	...
	require([ "dojo/dom", "gridx/Grid", "gridx/core/model/cache/Async",
			...
			"gridx/modules/ColumnLock",  "dojo/date/locale","dojo/domReady!" ], function(dom, Grid,
		...
		grid = new Grid({
			...
			pageSize : 25,
			sortInitialOrder: [{colId: "form_time_actual", descending: true}],
							...
							},
							{
								...
								decorator: function(cellData, rowId, rowIndex){
									if(isEmpty(cellData)){
										...
										var dateString = locale.format(dt,{selector: "date",datePattern: "yyyy-MM-dd"}); 
										return dateString;
									}
								}
							},
