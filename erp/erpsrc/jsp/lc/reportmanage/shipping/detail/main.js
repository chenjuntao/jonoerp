...
function getAppendValue() {
	...
	var appendValue = " AND h.FORM_TYPE in ('INNER_UNIFIED','INNER_CROSS')  AND h.PROVIDER_ID = '" + loginBranchId
			+ "' ";
	var nowType = dojo.byId("nowType").value;
	if (!isEmpty(nowType)) {
		if (nowType == "onWay") {
			appendValue += "  AND h.ON_STATUS='在途'";
		} else if (nowType == "inputed") {
			appendValue += "  AND h.INPUT_STATUS='已入库'";
		} else if (nowType == "different") {
			appendValue += "  AND h.DIFFERENT_STATUS='已处理差异'";
		}
	}
