...
define("dijit/layout/TabController", [
	...
	var TabController = declare("dijit.layout.TabController", StackController, {
		...
		postCreate: function(){
			...
			closeMenu.addChild(new MenuItem({
				label: _nlsResources.itemRefresh,
				ownerDocument: this.ownerDocument,
				dir: this.dir,
				lang: this.lang,
				textDir: this.textDir,
				onClick: function(evt){
					var currentItem = registry.byNode(this.getParent().currentTarget);
					var node = currentItem.page.containerNode;
					var ifr = dojo.query('div iframe',node);
					if (ifr.length > 0) {
						var location = ifr[0].contentWindow.location;
						location.reload();
					}
				}
			}));
			closeMenu.addChild(new MenuItem({
				label: _nlsResources.openNew,
				ownerDocument: this.ownerDocument,
				dir: this.dir,
				lang: this.lang,
				textDir: this.textDir,
				onClick: function(evt){
					...
					if (ifr.length > 0) {
						var location = ifr[0].contentWindow.location;
						window.open(ifr[0].src);
					}
				}
			}));
			closeMenu.addChild(new MenuItem({
				label: _nlsResources.closeOthers,
				ownerDocument: this.ownerDocument,
				dir: this.dir,
				lang: this.lang,
				textDir: this.textDir,
				onClick: function(evt){
					var tabs = controller.getChildren();
					var currentItem = registry.byNode(this.getParent().currentTarget);
					var container = registry.byId(controller.containerId);
					dojo.forEach(tabs, function(item){
						var page = item.page;
						if (currentItem.id != item.id && page.closable) {
